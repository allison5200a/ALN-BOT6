	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Iae @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}* por favor não seja um ghost <3`
		...
		try {
			...
			mess = {
				...
				only: {
					...
					benned: '*ᴅᴇsᴄᴜʟᴘᴇ sᴇᴜ ɴᴜ́ᴍᴇʀᴏ ᴇsᴛᴀ́ ᴘʀᴏɪʙɪᴅᴏ, ʟɪɢᴜᴇ ᴀᴏ ᴘʀᴏᴘʀɪᴇᴛᴀ́ʀɪᴏ ᴘᴀʀᴀ ᴛɪʀᴀʀ ᴏ sᴇᴜ ʙᴀɴ*',
		...
		case 'daftar':
			...
			if (isBanned) return reply(mess.only.benned)
				...
				ben = '```Lista de bans``` :\n'
					...
					break 
				case 'premiumlist':
				prem = '```Lista de usuários premium M K S B O T``` :\n'
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break  
                 ...
                 case 'kalkulator':
				   ...
				   if (!isUser) return reply(mess.only.userB)
				   if (isLimit(sender)) return reply(limitend(pushname2))
				     if (args.length < 1) return reply(`[❗] Enviar pedidos *${prefix}calculadora [ Angka ]*\nContoh : ${prefix}kalkulator 12*12\n*NOTE* :\n- Untuk Perkalian Menggunakan *\n- Untuk Pertambahan Menggunakan +\n- Untuk Pengurangan Mennggunakan -\n- Untuk Pembagian Menggunakan /`)
				    mtk = `${body.slice(12)}`
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (args.length < 1) return reply('Quem você quer verificar Man??')
					const can =['10% banyak" feião men kakaka:v\nAlvo de piadas:v','30% pow man cuida de si mesmo><','20% Sim espírito kajsjfjd👍','40% Wahh Kaká><','50% kaka cantik deh><','60% Ei linda🐊','70% Oi linda oi irmã🐊','62% Kakak Cantik><','74% Kakak isso é bonito><','83% Gostosarrr Kakak><','97% oloko🐊','100% Kakak Use o implante??:v','29% Espírito Kakak:)','94% Ei linda><','75% Oi linda irmã','82% wihh irmão deve frequentemente cuidar certo??','41% Espírito:)','39% Mais espirituoso🐊']
					const tik = can[Math.floor(Math.random() * can.length)]
					client.sendMessage(from, 'Questão : Lindo cheque irmão *'+cantik+'*\n\nPersen Kecantikan : '+ tik +'', text, { quoted: mek })
					break
				case 'ban':
					client.updatePresence(from, Presence.composing) 
				...
				case 'addprem':
					...
					reply(`*Sucesso! adicionado ${addpremium} Agora é um usuário premium*\n\nAproveite os recursos Premium:)`)
					break
				case 'removeprem':
					if (!isOwner) return reply(mess.only.ownerB)
					rprem = body.slice(13)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
                ...
                client.sendMessage(from, `\`\`\`✓Alteração das descrições do grupo com sucesso\`\`\` *${groupMetadata.subject}* Menjadi: *${body.slice(9)}*`, text, {quoted: mek})
                break
				case 'tts':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limitend(pushname2))
				    if (args.length < 1) return client.sendMessage(from, 'Código do idioma???', text, {quoted: mek})
				    if (args.length < 2) return client.sendMessage(from, 'Texto que você deseja traduzir??', text, {quoted: mek})
				    ts = body.slice(4)
				    kode = ts.split("/")[0]
				    teks = ts.split("/")[1]
         ...
         case 'lirik':
			    ...
			    case 'nsfwneko':
				    ...
				    if (isLimit(sender)) return reply(limitend(pushname2))
						...
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'rapaaaazz'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
				...
				case 'nsfwtrap':
				    ...
				    if (isLimit(sender)) return reply(limitend(pushname2))
						...
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'rapaaaaaaz kkkkk'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
				...
				case 'hentai':
				    ...
				    if (isLimit(sender)) return reply(limitend(pushname2))
						...
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'essa é gostosaar'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
				reply(mess.wait)
					if (args.length < 1) return reply('O que é Canção de Acorde??')
					tels = body.slice(7)
...
// only grup fitur anime
              case 'anime':
                ...
                if (!isAnime) return reply(' *Harus Mengaktifkan Mode Anime* ')
					...
					client.sendMessage(from, pok, image, { quoted: mek , caption: 'morraaaaa'})
					await limitAdd(sender) 
					break  
				case 'animekiss':
                if (isBanned) return reply(mess.only.benned)    
...
// akhir fitur anime
			...
			case 'hekerbucin':
				...
				case 'tiktokstalk':
					...
					if (!isUser) return reply(mess.only.userB)
					if (isLimit(sender)) return reply(limitend(pushname2))
					if (args.length < 1) return client.sendMessage(from, 'Cadê a porra do usuário?', text, {quoted: mek})
...
//akhir kreator
			   ...
			   case 'igstalk':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					quotes = `Citações de: *${anu.result.by}*\n\n\n*${anu.result.quote}*`
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('A marca-alvo que você deseja tirar!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
				...
				case 'kicktime':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					setTimeout( () => {
					client.sendMessage(from, 'Sem Sama "Al-fatihah', text)
					}, 8000)
					setTimeout( () => {
					reply('sucesso min:D')
					}, 7000)
					...
					setTimeout( () => {
					reply('Pedidos recebidos emitido:D')
					}, 0)
					break
				case 'promote':
				case 'pm':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (mentioned.length > 1) {
						...
						mentions(`Perintah di terima, @${mentioned[0].split('@')[0]} Você se torna um administrador no grupo*${groupMetadata.subject}*`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				case 'delete':
				...
				case 'toimg':
				if (isLimit(sender)) return reply(limitend(pushname2))
					if (!isQuotedSticker) return reply(' só sticker eemmm')
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\` Modo ativado com sucesso simi no grupo\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						samih.splice(from, 1)
						fs.writeFileSync('./database/json/simi.json', JSON.stringify(samih))
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`✓ Modo ativado com sucesso nsfw di group\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
		...
		case 'happymod':
			...
			break
				default:
                  ...
                  reply(`Desculpe manino, o comando *${prefix}${command}* Não está registrado no *${prefix}menu* não cria comandos😖🤣`)
                  }
